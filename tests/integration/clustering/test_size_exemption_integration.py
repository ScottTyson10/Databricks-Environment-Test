...
@pytest.fixture(scope="function")
def warehouse_id():
    """Warehouse ID needed for SQL execution in size detection."""
    warehouse_id = os.getenv("DATABRICKS_WAREHOUSE_ID")
    if not warehouse_id:
        pytest.skip("DATABRICKS_WAREHOUSE_ID not set - cannot test size detection")
    return warehouse_id
...
class TestSizeExemptionIntegration:
    ...
    def test_discovery_finds_all_test_tables(self, size_exemption_test_tables, integration_discovery):
        ...
        exemption_test_tables = [
            table for table in discovered_tables if table.table.startswith("size_exemption_test_")
        ]
    ...
    def test_small_table_size_detection(self, size_exemption_test_tables, integration_discovery, clustering_validator, warehouse_id):
        ...
        for table in small_tables:
            ...
            # Test size detection
            is_small = clustering_validator.is_small_table(table, warehouse_id=warehouse_id)
            assert is_small is True, f"Small table {table.table} should be detected as small"

            # Test exemption based on size
            is_exempt = clustering_validator.is_exempt_from_clustering_requirements(table, warehouse_id=warehouse_id)
            assert is_exempt is True, f"Small table {table.table} should be exempt from clustering"

    def test_large_table_size_detection(self, size_exemption_test_tables, integration_discovery, clustering_validator, warehouse_id):
        ...
        large_tables = [
            table for table in discovered_tables
            if table.table.startswith("size_exemption_test_") and "large" in table.table and "excluded" not in table.table
        ]
    ...
    def test_manual_exclusion_overrides_size(self, size_exemption_test_tables, integration_discovery, clustering_validator, warehouse_id):
        ...
        for table in manually_excluded_tables:
            ...
            assert clustering_validator.has_cluster_exclusion(table) is True, f"Table {table.table} should have manual exclusion flag"
            ...
            # Test that manual exclusion takes precedence
            exemption_without_size = clustering_validator.is_exempt_from_clustering_requirements(table)  # No warehouse_id
            assert exemption_without_size is True, f"Manual exclusion should work without size check for {table.table}"

    def test_clustered_table_not_exempt(self, size_exemption_test_tables, integration_discovery, clustering_validator, warehouse_id):
    ...
    def test_empty_table_exemption(self, size_exemption_test_tables, integration_discovery, clustering_validator, warehouse_id):
        ...
        is_small = clustering_validator.is_small_table(empty_table, warehouse_id=warehouse_id)
        assert is_small is True, "Empty table should be detected as small"

        # Empty table should be exempt
        is_exempt = clustering_validator.is_exempt_from_clustering_requirements(empty_table, warehouse_id=warehouse_id)
        assert is_exempt is True, "Empty table should be exempt from clustering"

    def test_boundary_conditions(self, size_exemption_test_tables, integration_discovery, clustering_validator, warehouse_id):
        ...
        is_small = clustering_validator.is_small_table(boundary_table, warehouse_id=warehouse_id)
        # Should be small (under 1MB test threshold)
        assert is_small is True, "Boundary table should be small (under test threshold)"

        # Should be exempt
        is_exempt = clustering_validator.is_exempt_from_clustering_requirements(boundary_table, warehouse_id=warehouse_id)
        assert is_exempt is True, "Boundary table should be exempt"

    def test_size_detection_without_warehouse_id(self, size_exemption_test_tables, integration_discovery, clustering_validator):
        """Test graceful handling when warehouse_id is not available."""
        discovered_tables = integration_discovery.discover_tables()
        ...
        # Size detection should return False when no warehouse_id (cannot determine size)
        is_small = clustering_validator.is_small_table(test_table, warehouse_id=None)
        ...
        is_exempt = clustering_validator.is_exempt_from_clustering_requirements(test_table, warehouse_id=None)
        ...
        assert is_exempt == has_manual_exclusion, "Without warehouse_id, should fall back to manual exclusion only"
    ...
    def test_validator_method_consistency(self, size_exemption_test_tables, integration_discovery, clustering_validator, warehouse_id, table_type):
        ...
        is_exempt = clustering_validator.is_exempt_from_clustering_requirements(test_table, warehouse_id=warehouse_id)
        should_enforce = clustering_validator.should_enforce_clustering_requirements(test_table, warehouse_id=warehouse_id)
